...
<body>
    <form method="post" action="/team/casinnki/diagnose1111_CA.php">
        <p>得意分野を選択してください(複数選択可)<br>
            <input type="checkbox" name="worries[]" value="1" <?php if(empty($_SESSION['worries'])||in_array("1",$_SESSION['worries'])) echo 'checked'?>>恋愛面<br>
            <input type="checkbox" name="worries[]" value="2" <?php if(empty($_SESSION['worries'])||in_array("2",$_SESSION['worries'])) echo 'checked'?>>家族・親族<br>
            <input type="checkbox" name="worries[]" value="3" <?php if(empty($_SESSION['worries'])||in_array("3",$_SESSION['worries'])) echo 'checked'?>>勉強面<br>
            <input type="checkbox" name="worries[]" value="4" <?php if(empty($_SESSION['worries'])||in_array("4",$_SESSION['worries'])) echo 'checked'?>>メンタルヘルス（漠然とした不安）<br>
